...
public class sobrecargaMetodos {
    ...
    // SOBRECARGAR UN METODO ES HACER OTRO METODO CON EL MISMO NOMBRE PERO DIFERENTE CONTENIDO
    // METODO QUE REPRODUZCA UNA PELICULA DE MANERA ALEATORIA:

    public static String play(String[]movie){
        Random random = new Random();
        int index = random.nextInt(movie.length - 1); // CREA UN NUMERO ALEATORIO (LARGO DEL ARREGLO - 1 PORQUE ARRANCA EN 0)
        String title = movie[index];
        for(int i = 0; i <= 10; i++){
            System.out.println("Playing movie: " + title);
        }
